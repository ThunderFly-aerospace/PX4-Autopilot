...
void os_setExtendedTimedCallback (osxjob_t* xjob, osxtime_t xtime, osjobcb_t cb) {
    ...
    hal_enableIRQs();
#ifdef DEBUG_JOBS
    debug_printf("Scheduled job %u, cb %u at %t\r\n", (unsigned)xjob, (unsigned)cb, xtime);
#endif // DEBUG_JOBS
}
...
void os_waitForJobTime(osjob_t * j)
{

    if (j) {
        debug_printf("Sleeping until job %u, cb %u, deadline %t\r\n", (unsigned)j, (unsigned)j->func, (ostime_t)j->deadline);        
        if (hal_sleep(OS.exact ? HAL_SLEEP_EXACT : HAL_SLEEP_APPROX, j->deadline) == 0) 
        {
...
#ifdef DEBUG_JOBS
            debug_printf("Running job %u, cb %u, deadline %t\r\n", (unsigned)j, (unsigned)j->func, (ostime_t)j->deadline);
#endif // DEBUG_JOBS
#ifdef CFG_warnjobs
            if ( delta > 1 ) {
                debug_printf("WARNING: job %u (func %u) executed %d ticks late\r\n", (unsigned) j, (unsigned) j->func, delta);
            }
#endif
        }
